const AboutsectionDataPt = [
    {
        id: 1,
        section_title: "Sobre Nós",
        paragraph_1: "O Restaurante Nortada foi fundado em 1979 por Manuel Esteves da Silva e \
        Maria Elisa Esteves da Silva. Ao longo de mais de quatro décadas, este espaço icónico tem \
        sido gerido pela mesma família, que preserva e honra os valores dos seus fundadores: servir \
        com excelência, num ambiente familiar, acolhedor e autêntico.",
        paragraph_2: "O amor, a paixão e a dedicação dos seus fundadores e da atual gerência fizeram do \
        Restaurante Nortada um símbolo de excelência na costa de Sintra. Localizado num cenário privilegiado, \
        oferece uma vista deslumbrante e inesquecível sobre o Atlântico, tornando cada refeição numa \
        experiência única.",
        paragraph_3: "Especializado em peixes e mariscos sempre frescos, cuidadosamente selecionados, o \
        Restaurante Nortada é o local ideal para os amantes da gastronomia que procuram sabores genuínos, \
        aliados a um ambiente tranquilo e sofisticado. Cada prato reflete a essência da cozinha tradicional\
        portuguesa, combinada com a frescura dos melhores ingredientes do mar.",
        paragraph_4: "Além da gastronomia, o Restaurante Nortada proporciona momentos verdadeiramente mágicos, \
        como assistir a um pôr do sol que deixa qualquer um sem palavras, onde o horizonte se pinta de tons dourados e \
        laranja. Aqui, não será tratado apenas como um cliente, mas como parte da família.",
        paragraph_5: "Deixe-se envolver pela atmosfera calorosa e pelo cuidado de uma equipa dedicada a proporcionar \
        memórias inesquecíveis. No Restaurante Nortada, sentirá que encontrou o lugar perfeito à beira-mar.",
        paragraph_6: "Seja para uma refeição romântica a dois, um encontro de amigos ou uma celebração especial, o Restaurante \
        Nortada adapta-se a cada ocasião com um serviço atencioso e personalizado. O ambiente sereno e elegante, aliado à qualidade \
        excecional dos pratos, faz deste espaço um destino obrigatório para quem valoriza a boa gastronomia e a hospitalidade genuína."
    },
];

export default AboutsectionDataPt;
